<template>
  ...
  <span v-if="!$q.screen.lt.md">
    ...
    <template v-else v-for="(elem, idx) in elements" :key="idx">
      <q-btn
        class="q-ml-xs"
        v-if="elem.isShowBtn"
        :color="elem.color"
...
import { ref } from 'vue';
import { IPropsTableElements, TEvents } from '../table.interface';

defineProps<IPropsTableElements>();
const emit = defineEmits<{
  (event: TEvents, data: string, searchAll?: boolean): void;
}>();
